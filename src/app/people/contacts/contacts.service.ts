...
export class ContactsService {
    ...
    getContacts(): Observable<Contact[]> {
        return this._httpClient.get<any[]>('https://orgchart-api.ssa.gov/api/v2/person/lookup/all').pipe(

            map((data: any[]) => data.filter(record => record.objectId !== '' && record.objectId !== null && record.objectId !== '-')
                .map(x => ({
                    id: x.objectId,
                    office: x.officeName,
                    avatar: x.image,
            ...
            tap((contacts) => {
                const result = contacts.sort((a, b) => a.name < b.name ? -1 : 1);
    ...
    // map((data: any[]) => data.map(x => {
    //     if(x.identifiers.objectId !== '-'){
    //         return identifiers.objectId
    //     }
    // }),
    /**
    ...
    searchContacts(query: string): Observable<Contact[]> {
        ...
        if (query === null || query === undefined || query === '') {
            urlPath = 'https://orgchart-api.ssa.gov/api/v2/person/lookup/all';
        } else {
            urlPath = `https://orgchart-api.ssa.gov/api/v2/person/lookup/${query}/all`;
        }
        return this._httpClient.get<Contact[]>(urlPath, {
            params: { query }
        }).pipe(
            map((data: any[]) => data.filter(record => record.objectId !== '' && record.objectId !== null && record.objectId !== '-')
            .map(x => ({
    ...
    getContactById(id: string): Observable<Contact> {
        return this._httpClient.get<any>(`https://orgchart-api.ssa.gov/api/v2/person/${id}`)
            .pipe(
                map((data: any) => {
                    const contactUser = ({
                        id: data.identifiers.objectId,
                    ...
                    this._contact.next(contactUser);
